...
| observers | Object | 否 | 数据字段监听器，用于监听 data 的变化，可查看 [数据监听器](https://opendocs.alipay.com/mini/04y1n6)。 | [2.8.1](https://opendocs.alipay.com/mini/framework/lib-upgrade-v2) |
...
| ref | Function | 否 | 指定组件被 ref 引用时或被自定义组件选择方法（如 selectOwnerComponent ）选中时的返回值。 | **1.18.0** |
...
| createIntersectionObserver | Object | 创建 IntersectionObserver 对象实例。基础库 **2.7.4** 起支持。 |
| getTabBar | - | 获取自定义 tabBar 组件实例。基础库 **2.7.20** 起支持。 |
| selectOwnerComponent | - | 选取当前组件的创建者（即 AXML 中定义了此组件的组件），返回它的组件实例对象（会被 `ref` 影响）。基础库 **2.7.22** 起支持。 |
| selectComposedParentComponent | - | 选取当前组件在事件冒泡路径上的父组件，返回它的组件实例对象（会被 `ref` 影响）。基础库 **2.7.22** 起支持。 |
| $selectComponent | String | 根据传入的 [selector 匹配器](https://opendocs.alipay.com/mini/01og7z#selector%20%E8%AF%AD%E6%B3%95) 查询，返回匹配到的第一个的组件实例（会被 `ref` 影响）。基础库 **2.8.0** 起支持。 |
| $selectAllComponents | String | 根据传入的 [selector 匹配器](https://opendocs.alipay.com/mini/01og7z#selector%20%E8%AF%AD%E6%B3%95) 查询，返回匹配到的全部的组件实例数组（会被 `ref` 影响）。基础库 **2.8.0** 起支持。 |
