...
**说明：**`$spliceData` 自 1.7.2 之后才支持，可以使用 **my.canIUse('page.$spliceData')** 做兼容性处理，可查看 [兼容接口说明](/mini/framework/compatibility)。
...
**说明**：`$batchedUpdates` 自 1.14.0 之后才支持，可以使用 **my.canIUse('page.$batchedUpdates')** 做兼容性处理，可查看 [兼容接口说明](https://opendocs.alipay.com/mini/framework/compatibility)。
