小程序页面目录中的 `.json` 文件用于配置当前页面的窗口表现。页面配置比 app.json 全局配置简单得多，可用于覆盖 [app.json 应用配置 - window](https://opendocs.alipay.com/mini/framework/app-json#window) 的配置项，但无需写 `window` 这个键。

页面配置项会优先于全局配置项。

页面配置支持 [app.json 应用配置 - window](https://opendocs.alipay.com/mini/framework/app-json#window) 中所有的配置项，同时支持如下额外配置项：

- [注意：该配置即将废弃] 支持 `optionMenu` 配置导航图标，点击后触发 `onOptionMenuClick`。
...
其他配置项的详细文档请见：[app.json 应用配置 - window](https://opendocs.alipay.com/mini/framework/app-json#window)。
