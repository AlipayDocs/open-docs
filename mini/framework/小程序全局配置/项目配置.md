...
| typescript | Boolean | false | 小程序是否启用 typescript 支持，详见 [TypeScript 和 Less 编译](https://opendocs.alipay.com/mini/02zko2)。 |
| less | Boolean | false | 小程序是否启用 less 支持，详见 [TypeScript 和 Less 编译](https://opendocs.alipay.com/mini/02zko2)。 |
...
| globalObjectMode | String | - | 小程序全局对象（global/globalThis）访问策略, 详细说明请查看 [globalObjectMode](https://opendocs.alipay.com/mini/03dbc3#globalObjectMode)|
| transpile | Object | - | 小程序代码编译至 ES5 配置, 详细说明请查看 [transpile](https://opendocs.alipay.com/mini/03dbc3#transpile)|
...
#### 使用要求
- 小程序开发者工具 IDE 3.8.0 以上版本
- 命令行工具 CLI 2.0.0 及以上版本

#### globalObjectMode 可选值
| 值 | 含义 |
| --- | --- |
| legacy | 保持历史行为不变 |
| enable | （推荐）可访问到真实的小程序 JavaScript 全局对象  global 与 globalThis，可最大化兼容当前 npm 生态 |
| fake | 可访问到一个全局挂载的虚拟空对象，可兼容部分 npm 生态，或基于其实现全局的模块管理等。|
...
#### 配置
- ignore: 默认值：[]，开发者可以通过配置 glob 规则跳过某些文件/目录的转译
...
#### ignore 规则解释
ignore 规则是 glob，相对于 mini.project.json 所在的目录，同时支持前缀的 ! 来表示不被 ignore，例如
```json
{
  "transpile": {
    "script": {
...
意为忽略当前目录下的 node_modules/ 下的所有文件，但不忽略 node_modules/lodash-es/ 下面的文件
...
| parallel | Boolean | true | 是否开启多进程构建。小程序默认使用多进程编译，如果资源占用过大，可以通过 disableParallel 关闭多进程。 |
| sourcemap | Boolean | true | 是否开启生成 sourcemap。本地开发时默认开启 sourcemap ，如果资源占用过大，可以通过 disableSourcemap 关闭 sourcemap。 |
| minify | Boolean | true | 是否开启代码压缩。如果构建耗时过长，或者资源占用过大导致构建失败，可以通过 minify: false 关闭代码压缩。 |
