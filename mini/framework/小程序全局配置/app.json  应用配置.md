...
```
**注意**：此特性从基础库 [2.7.20](https://opendocs.alipay.com/mini/framework/lib-upgrade-v2)，[IDE 3.1.2](https://opendocs.alipay.com/mini/ide/download) 开始支持。若强依赖此特性，建议设置最低基础库版本号为2.7.20。否则，在低版本的基础库，会因为无法识别正确的首页而导致地渲染出『小房子』。
...
**注意**：tabBar items 数组第一个元素的 pagePath 必须是 [entryPagePath](https://opendocs.alipay.com/mini/framework/app-json#entryPagePath) 指定的默认启动路径， 否则 tabBar 配置不生效。
