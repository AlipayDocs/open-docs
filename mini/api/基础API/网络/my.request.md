...
| 12 | 网络出错。 | <ul>建议检查网络环境是否正常，服务器是否稳定。</ul> |
| 13 | 超时。 | <ul>建议检查网络环境是否正常，服务器是否正常响应，若请求需要时间长，可适当设置超时时间  timeout。</ul> |
...
| 15 | 传参失败。 | <ul>小程序页面传参如果做 urlencode 需要把整体参数进行编码。</ul> |
...
| 20 | 请求已被停止/服务端限流。 | <ul>请确认请求服务器是否能正常请求和响应。</ul> |
|  | 请求 URL 不支持 HTTP，请使用 HTTPS。 | <ul>小程序已经不支持 HTTP 请求，请使用 HTTPS。</ul> |
| 23 | 代理请求失败。 | <ul>建议检查代理配置是否正确。</ul> |
