...
- 覆盖安装支付宝（不是先删除再安装）、关闭小程序，这两种操作均不会导致小程序缓存失效。<br />开发者调用 API 存储的缓存数据，需要自行调用对应的[删除](https://opendocs.alipay.com/mini/api/of9hze)/[清除](https://opendocs.alipay.com/mini/api/storage) API 进行删除。<br />长期未使用或在应用中心删除的小程序的缓存数据会被系统清理。
