...
| **属性** | **类型** | **必填** | **描述** |
| -------- | -------- | -------- | -------- |
| url     | String   | 否      | 网格贴图地址描述。'type = 1'，url 和图片路径格式需为 '{url}&x={x}&y={y}&z={z}'。为 0 时, 服务器图片路径格式为 '{url}/{z}/{x}-{y}-{z}.png'。 其中 xyz 为贴图的 x 坐标，y 坐标，和缩放级别 zoom。 |
| type | Number    | 否       | 贴图地址转换类型。默认 'type = 1'。 |
