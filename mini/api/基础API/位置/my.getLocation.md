...
| type | Number | 否 | 获取经纬度数据的类型。默认值为 0。<ul><li>0：获取经纬度。</li><li>1：获取经纬度和省市区县数据。</li><li>2：获取经纬度、省市区县和街道数据，不推荐使用。（不推荐使用的原因：精度过高，接口返回的速度会变慢。）</li><li>3：获取经纬度、省市区县、街道和详细到 POI 级别的数据，不推荐使用。不推荐使用的原因：精度过高，接口返回的速度会变慢。</li></ul> |
