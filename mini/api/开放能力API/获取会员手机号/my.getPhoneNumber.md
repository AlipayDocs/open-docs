...
目前该功能需要在开发者后台完成敏感信息申请后才可以使用，入口为 **开发** > **产品绑定** > **绑定产品** > **获取会员手机号** > **用户信息申请**，此功能需谨慎使用，若支付宝发现信息存在超出约定范围使用或者不合理使用等情况，支付宝有权永久回收该小程序的该接口权限。

在开放平台控制台 > 开发设置中配置 **接口加密方式** 后，`my.getPhoneNumber` 才能返回用户加密的手机号（否则将报错 40001，“缺少加密配置项”）。
...
A：
- 请检查小程序已添加 **获取会员手机号** 功能包，并已在 **隐私内容申请** 中 **申请手机号**（若在小程序详情页看不到敏感信息申请的入口，请使用主账号登录）。申请路径为：[开放平台控制台](https://open.alipay.com/dev/workspace) > 选择需要配置的应用，点击进入应用详情页 > **开发** > **产品绑定**，绑定 **获取会员手机号** 产品 > **用户信息申请**。
