...
```
从基础库 [2.8.9](https://opendocs.alipay.com/mini/framework/lib-upgrade-v2)、[IDE 2.7.1](https://opendocs.alipay.com/mini/ide/download) 起，可以通过 app.json 的配置项为插件页面指定抽象组件实现。例如，要给插件名为 `plugin-index` 的页面中的抽象节点 `mp-view` 指定宿主小程序的自定义组件 `/components/comp-from-miniprogram` 作为实现的话：
```json
{
  "plugins": {
    "myPlugin": {
